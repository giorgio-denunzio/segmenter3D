...
*/
    ...
    connect (ui->horizontalSlider_p, SIGNAL (buttonClicked (int)), this, SLOT (pSliderValueChanged (int)));
    ...
    // I don't have to explicitly connect.
    ...
    if (clusters.size() > 0)
    {
        ...
        for (int c = 0; c < clusters.size(); c++)
        {
            ...
            if (selectedClustersFlags[c])
            {
                ...
                {
                    labeledCloud->points[clusters[c].indices[k]].r = 255;
...
void PCLViewer::on_buttonClicked_LoadSparseOrDenseCloud(int id)
{
    ...
    if (FileName.isNull())   // or isEmpty()
        ...
        if (loadCloud(FileNameStr))
        {
            ...
            //////////////////////////////////
            // This loop was used to check if the formato of the cloud in memory is the same as the obj or ascii ply on disk, as to the order of vertices, and it is.
            //     for (int i = 0; i < 100; i++)
            //    {
...
void PCLViewer::on_pushButton_StartSegmentation_clicked()
{
    ...
    case 1: // Region Growing
    {
        ...
        int neKSearch                 = stoi(ui->lineEdit_neKSearch->text().toStdString());
        int rgMinClusterSize          = stoi(ui->lineEdit_rgMinClusterSize->text().toStdString());
        int rgMaxClusterSize          = stoi(ui->lineEdit_rgMaxClusterSize->text().toStdString());
        int rgNumberOfNeighbours      = stoi(ui->lineEdit_rgNumberOfNeighbours->text().toStdString());
        float rgSmoothnessThreshold   = stof(ui->lineEdit_rgSmoothnessThreshold->text().toStdString());
        float rgCurvatureThreshold    = stof(ui->lineEdit_rgCurvatureThreshold->text().toStdString());
        CSegmenterRegionGrowing srg(xyzCloud, neKSearch, rgMinClusterSize, rgMaxClusterSize, rgNumberOfNeighbours, rgSmoothnessThreshold, rgCurvatureThreshold);
        labeledCloud = srg.getOutputCloud();
        reg = srg.getOutputRegionGrowing();
        clusters = srg.getOutputClusters();
