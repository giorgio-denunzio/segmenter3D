...
struct status
{
	...
	std::vector <float> clusterColors;
	std::vector<bool> deletedClusterFlags;
	std::vector<bool> selectedClusterFlags;
...
//  std::vector <pcl::RGB> clusterColors;               // Contains the color components of the segmented clusters, so that I can restore the clusters if I first delete them (by coloring in red)
  ...
  struct status status;    // where to save the former status, for a 1-level undo
  ...
  std::vector<bool> deletedClusterFlags;    // true/false if cluster deleted or not
  std::vector<bool> selectedClusterFlags;    // true/false if cluster selected or not
...
private slots:
  ...
  void on_pushButton_UndoSegmentation_clicked();
...
private:
  ...
  pcl::IndicesPtr extractSelectedClustersIndices(void);
  PointCloudT::Ptr extractSelectedPointsFromCloudByIndices(pcl::IndicesPtr indicesToPreserve);
  void dropSelectedClustersFromClusters(void);
  ...
  void segmResetAllDefaults(void);
  void segmRGResetDefaults(void);
  void segmLCCPResetDefaults(void);
  void writeOutClusterFlags(void);
  void writeOutClusterInfo(void);
