...
void PCLViewer::resetAll()
{
	...
	ui->pushButton_UndoSegmentation->setEnabled(false);
...
// I might define some deleteACluster similarly to selectACluster and simplify the code but perhaps it is useless
// because I delete clusters only in this function (IIRC)
{
	...
	if (clusters.size() > 0)
	{
		// Points belonging to the selected clusters (checked in the selectedClusterFlags vector) get colored in red,
		//   and the corresponding cluster position in deletedClusterFlags are marked as deleted so that after a
		...
		for (int c = 0; c < clusters.size(); c++)
		{
			...
			if (selectedClusterFlags[c])
			{
				...
				deletedClusterFlags[c] = true;    // logically delete the cluster (set the "selected cluster" flag)
				// selectedClusterFlags[c] = false;  // logically deselect the cluster (clear the "selected cluster" flag) (redundant, see call of on_pushButton_DeselectAll_clicked)
...
void PCLViewer::on_pushButton_SelectAll_clicked()
{
	...
	for (int c = 0; c < clusters.size(); c++)
		if (!selectedClusterFlags[c] && !deletedClusterFlags[c])
			selectACluster(c);
	writeOutClusterFlags();
}
...
//	for (int c = 0; c < clusters.size(); c++)
//		selectedClusterFlags[c] = !selectedClusterFlags[c];
...
void PCLViewer::on_pushButton_Restore_clicked()
{
	...
	for (int c = 0; c < clusters.size(); c++)
		if (deletedClusterFlags[c])   // the cluster had been logically deleted
		{
			...
			deletedClusterFlags[c] = false;  // the cluster is now not logically deleted
		}
...
void PCLViewer::on_pushButton_LoadMesh_clicked()
{
	...
	if (FileName.isNull())   // or isEmpty()
		...
		selectedClusterFlags.clear();
		deletedClusterFlags.clear();
...
void PCLViewer::on_buttonClicked_LoadSparseOrDenseCloud(int id)
{
	...
	if (FileName.isNull())   // or isEmpty()
		...
		if (loadCloud(FileNameStr))
		{
			...
			// Enable segmentation
			//ui->pushButton_StartSegmentation->setEnabled(true);   // No, only after selecting some object!
...
void PCLViewer::on_pushButton_StartSegmentation_clicked()
{
	...
	cout << "PCLViewer::on_pushButton_StartSegmentation_clicked starting\n";

	// Save current status:
	// clusters, clusterColors, deletedClusterFlags, selectedClusterFlags
	// The labeledCloud is reconstructed from the others.
	...
	PointCloudT::Ptr selectedCloud(new PointCloudT);        // cloud to be passed to the segmentation functions (this will be the selected cluster[s]; the whole model if no clusterization has been made yet); replaces "labeledCloud" and will be inserted in it...
															// Verify if th new PointCloudT is necessary!!!!
	pcl::IndicesPtr iPtr = extractSelectedClustersIndices();
	selectedCloud = extractSelectedPointsFromCloudByIndices(iPtr);
	//if (selectedCloud->points.size() == 0)  // If nothing is selected, it means everything selected
	...
	std::vector <pcl::PointIndices> localClusters;   // will contain the segmented clusters, replaces "clusters" locally, and will be inserted in "clusters" after removing the clusters which undergo segmentation
	...
	case 1: // Region Growing
	{
		...
		if (!badInput)
		{
			...
			selectedCloud = seg.getOutputCloud();
			////////reg = seg.getOutputRegionGrowing();
			localClusters = seg.getOutputClusters();
		}
	...
	if (!badInput)
	{
		...
		// Now update "clusters" by deleting the selected clusters (which have been segmented, so they exist no more) and then by inserting the newly created clusters
		// We must remember that the indices of the cluster points were contained in iPtr, but now have changed because iPtr has been used to filter the original cloud and to
		// build a subset of it; however, the first indix in iPtr is also the true original index of the point that has index 1 in localClusters; so I have to filter the new indices with the old ones.

		dropSelectedClustersFromClusters();   // delete the selected clusters (which were passed to the segmentation code)

		// localClusters is a vector of PointIndices:  vector <PointIndices>
		
		// Now insert the new clusters, filtering the point indices by iPtr

		for (int c = 0; c < localClusters.size(); c++)   // for each returned cluster
		{
			for (int i = 0; i < localClusters[c].indices.size(); i++)   // for each index contained in the c-th cluster, use iPtr corresponding value (the original index, i.e. the index in the original cloud)
			{
				//int k = iPtr->operator[](10);
				int *ip = &localClusters[c].indices[i];   // point to the i-th index in the c-th cluster, and...
				*ip = iPtr->operator[](*ip);              // .. replace it with its original value (the one used for filtering in the original cloud before submitting to segmentation) passing through iPtr
			}
			clusters.push_back(localClusters[c]);
		}

		// Now insert the points with their new color, into the point cloud

		for (size_t i = 0; i < selectedCloud->points.size(); ++i)
		{
			int labClIdx = iPtr->operator[](i);
			labeledCloud->points[labClIdx].rgb = selectedCloud->points[i].rgb;
		}

		// Now synchronize the various structures: clusterColors, deletedClusterFlags and selectedClusterFlags

		clusterColors.clear();
		deletedClusterFlags.clear();
		selectedClusterFlags.clear();

		// Fill the selectedClusterFlags and the deletedClusterFlags with "false", and clusterColors with the colors taken from the cloud according to the different clusters
		...
		for (int c = 0; c < clusters.size(); c++)
		{
			...
			deletedClusterFlags.push_back(false);
			selectedClusterFlags.push_back(false);
		}
		...
		ui->pushButton_UndoSegmentation->setEnabled(true);
	}
...
void PCLViewer::on_pushButton_UndoSegmentation_clicked()
{

	on_pushButton_DeselectAll_clicked();

	clusters = status.clusters;
	clusterColors = status.clusterColors;
	deletedClusterFlags = status.deletedClusterFlags;
	selectedClusterFlags = status.selectedClusterFlags;
	clusterLabels = status.clusterLabels;

	// Now I have to set colors into the labeledCloud point cloud AND to refresh the screen as to selections and labels

	for (int c = 0; c < clusters.size(); c++)   // for each cluster
		for (int j = 0; j < clusters[c].indices.size(); j++)   // for each point in the cluster
			labeledCloud->points[clusters[c].indices[j]].rgb = clusterColors[c];


	// Refresh labels, I don't know where!!!!!!
	...
	// The next instructions are redundant because I know that there are the clusters prepared for segmentation, selected 
	//if (isAnythingSelected())
	//	ui->pushButton_StartSegmentation->setEnabled(true);
	//else
	//	ui->pushButton_StartSegmentation->setEnabled(false);
	...
	// Now the button must be disabled (just ONE level of undo)
	ui->pushButton_UndoSegmentation->setEnabled(false);
...
void PCLViewer::on_pushButton_ClearSegmentation_clicked()
{
	...
	// - initialize the deletedClusterFlags vector to one place, false
	// - initialize the selectedClusterFlags vector to one place, false
...
void PCLViewer::pp_callback(const pcl::visualization::PointPickingEvent& event, void* viewer_void)
{
	...
	// The variable member "deletedClusterFlags" tells if the custer was "deleted" so it must not be searched
	...
	// in the selectedClusterFlags vector, otherwise select it (add a cube) and set true in selectedClusterFlags
	if (selectedClusterFlags[theCluster])    // also if (viewer->contains(ss.str()))
	{
...
void PCLViewer::selectACluster(int theCluster)
{
	if (theCluster >= 0 && theCluster < clusters.size())
	{
		...
		selectedClusterFlags[theCluster] = true;
...
void PCLViewer::deSelectACluster(int theCluster)
{
	if (theCluster >= 0 && theCluster < clusters.size())
	{
		...
		selectedClusterFlags[theCluster] = false;
		// Refresh (not always needed...)
		ui->qvtkWidget->update();
		if (isAnythingSelected())
			ui->pushButton_StartSegmentation->setEnabled(true);
...
pcl::IndicesPtr PCLViewer::extractSelectedClustersIndices(void)
{
	...
	for (int j = 0; j < selectedClusterFlags.size(); j++)
		if (selectedClusterFlags[j])
		{
	...
	std::sort(indicesToPreserve->begin(), indicesToPreserve->end());    // sort, to be sure we have always the same list of indices
	return indicesToPreserve;
}


PointCloudT::Ptr PCLViewer::extractSelectedPointsFromCloudByIndices(pcl::IndicesPtr indicesToPreserve)
{
...
void PCLViewer::dropSelectedClustersFromClusters(void)
{
	// Must go backwards!!!!!!! Otherwise when I erase/drop a cluster, the following ones are renumbered!!
	for (int j = selectedClusterFlags.size() - 1; j >= 0; j--)
		if (selectedClusterFlags[j])
		{
			deSelectACluster(j);
			dropClusterFromClustersWithIndex(j);
		}
}
...
// NOT USED???
// Conversion from pcl::IndicesPtr to pcl::PointIndices::Ptr
...
bool PCLViewer::isAnythingSelected(void)
{
	bool or = false;
	for (int j = 0; j < selectedClusterFlags.size(); j++)
		or |= selectedClusterFlags[j];
	return or ;
}


void PCLViewer::writeOutClusterFlags(void)
{
	cout << "selectedClusterFlags is:";
	for (int c = 0; c < clusters.size(); c++)
		cout << " " << selectedClusterFlags[c];
	cout << '\n';
	cout << "deletedClusterFlags is:";
	for (int c = 0; c < clusters.size(); c++)
		cout << " " << deletedClusterFlags[c];
	cout << '\n';
}


void PCLViewer::writeOutClusterInfo(void)
{
	for (int c = 0; c < clusters.size(); c++)
	{
		int clusterSize = clusters[c].indices.size();
		std::cout << " Cluster #" << c << " size is " << clusterSize << std::endl;
		std::cout << "    First point indices are: ";
		int ptLim = clusters[c].indices.size() < 10 ? clusters[c].indices.size() : 10;
		for (int j = 0; j < ptLim; j++)
			std::cout << clusters[c].indices[j] << " ";
		std::cout << std::endl;
	}
	std::cout << std::endl;
}
...
// Save current status (for 1-level undo):
// clusters, clusterColors, deletedClusterFlags, selectedClusterFlags
// The labeledCloud is reconstructed from the others.

void PCLViewer::saveCurrentStatus()
{
	status.clusters = clusters;
	status.clusterColors = clusterColors;
	status.deletedClusterFlags = deletedClusterFlags;
	status.selectedClusterFlags = selectedClusterFlags;
	status.clusterLabels = clusterLabels;
}
