...
// To do! 
// 1) delete old plane, cylinder etc files
// 

// cmake -G "Visual Studio 14 2015 Win64" ..
// Remember to add pgmb_io.* to the solution, before compiling/linking
// project J:\SFM\gerrard-hall\GH_UltraHigh_Res_photoscan.obj
// project D:\_CORVALLIS_\gerrard-hall\GH_Medium_Res_MeshLab.ply


// Remember to fix the plan choice, depending on the loaded model!!! See " Now just state the floor is" at about half code
...
// http://stackoverflow.com/questions/36380217/pclpclpointcloud2-usage
...
bool visualizeClouds(boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer, pcl::PointCloud<PointT>::Ptr cloud_rotated, pcl::PointCloud<PointT>::Ptr cloud_projected, PointT minPt, PointT maxPt, float binSide);
bool segmentCylinders(unsigned int nc, float binSide);


// All'indirizzo
// http://pointclouds.org/documentation/tutorials/reading_pcd.php
// si parla (poco) delle differenze tra PointCloud e PointCloud2
...
int main(int argc, char** argv)
{
	...
	pcl::PCLPointCloud2::Ptr cloud_blob(new pcl::PCLPointCloud2),  // point cloud structure for model loading
					         cloud_filtered_blob(new pcl::PCLPointCloud2); // point cloud structure for downsampled model
	...
	// Visualiza cloud, projected cloud, bounding boxes for cylinders
...
// Accesso alle coordinate di un punto
...
bool applyHoughToPointDensityOfProjectedCloud(unsigned int &nc)
{
	...
	// and other links; some say that "system" is not safe!

	system("findcolumnloop.exe");  // reads data.pgm and saves centers.txt and radii.txt
...
bool visualizeClouds(boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer, pcl::PointCloud<PointT>::Ptr cloud_rotated, pcl::PointCloud<PointT>::Ptr cloud_projected, PointT minPt, PointT maxPt, float binSide)
{
	...
	*/
	...
	// I already know x and y (they coincide with those of the cloud_projected) but I also want z limits
	...
	while ((centersFile >> cx >> cy) && (radiiFile >> cr))
	{
		...
		viewer->addCube(minPoint[0], maxPoint[0], minPoint[1], maxPoint[1], minPoint[2], maxPoint[2], 0.0, 1.0, 0.0, str);
